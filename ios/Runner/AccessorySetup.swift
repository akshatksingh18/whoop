...
enum AccessorySetup {
  ...
  // response, so the 16-bit SIG member UUID 0xFD4B gives gen5 a second chance
  // to match (what still fits a 31-byte AD). `fileprivate` so Impl can read it.
  // Every criterion used in an ASDiscoveryDescriptor must also be listed in
  // Info.plist; this one is not a registry service, so it is deliberately not
  // generated by `tool/gen_ios_ask_plist.dart`.
...
private final class Impl {
  ...
  private var activationWaiters: [(Result<Void, PickerError>) -> Void] = []
  ...
  /// Run [completion] only after AccessorySetupKit has delivered `.activated`.
  ///
  /// Apple requires picker/session work to wait for that event. Keeping the small
  /// waiter queue here also makes the first `provisionedId` call and the first
  /// `showPicker` call safe when they arrive in the same cold launch.
  private func ensureActivated(
    _ completion: @escaping (Result<Void, PickerError>) -> Void
  ) {
    queue.async { [weak self] in
      guard let self = self else {
        completion(.failure(PickerError(message: "AccessorySetupKit is unavailable.")))
        return
      }
      ...
      self.activationWaiters.append(completion)
      guard !self.activationRequested else { return }
      self.activationRequested = true
      self.session.activate(on: self.queue) { [weak self] event in
        self?.onEvent(event)
      }
    }
  }
  ...
  private func onEvent(_ event: ASAccessoryEvent) {
    ...
    case .invalidated:
      ...
      activationRequested = false
      let error = PickerError(message: "AccessorySetupKit session was invalidated.")
      let waiters = activationWaiters
      activationWaiters.removeAll()
  ...
  func provisionedIds(_ completion: @escaping ([String]) -> Void) {
    ensureActivated { [weak self] activation in
  ...
  func showPicker(addAnother: Bool = false,
    ...
    ensureActivated { [weak self] activation in
      ...
      self.showPickerWhenActivated(addAnother: addAnother, completion)
    }
  }

  private func showPickerWhenActivated(
    addAnother: Bool,
    _ completion: @escaping (Result<String, PickerError>) -> Void
  ) {
    ...
    // The extra gen5-only 16-bit member-UUID fallback is appended below.
  ...
  ///
  /// If iOS rejects the widened descriptor list, retry once with the WHOOP 4.0
  /// item that already ships, so the fallback can never take down 4.0 pairing.
  ///
  ...
  func removeAll(_ completion: @escaping () -> Void) {
    ensureActivated { [weak self] activation in
      ...
      let accessories = self.session.accessories
      guard !accessories.isEmpty else { completion(); return }
      let group = DispatchGroup()
      for acc in accessories {
        group.enter()
        self.session.removeAccessory(acc) { _ in group.leave() }
      }
