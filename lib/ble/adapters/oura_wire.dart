...
// is exercised by `test/adapters/oura_wire_test.dart` against real captured
// records with no hardware in the room.
//
...
OuraFrame? parseFrame(List<int> value) {
...
OuraEvent? parseEvent(OuraFrame f) {
...
// thing stopping them. `GattBandLink`'s dangerous-opcode block reads the opcode
// out of a WHOOP envelope and answers null for an unframed band, so it does NOT
// cover this ring — nothing at the link refuses these. The ring has a factory
// reset, a firmware-update mode, a DFU state machine, a flight mode, a
// manufacturing-mode setter and a bulk-sampler channel with an erase operation.
// There is no builder for any of them here, `oura.dart` writes nothing it did
