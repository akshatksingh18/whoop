...
DecodedSample? decodeRecord(String hex) {
  ...
  if (kKnownRecordVersions.contains(recType)) {
    ...
    return DecodedSample(
      ...
      // `hr > 0` is kept for that reason -- but note the parity oracle cannot
      // EVIDENCE it, because that column was generated by the same `hr > 0`
      // expression (ts/live.ts). Confirming it needs captures with
      // independently known wear state.
